...
class ObjectDB extends Database
{
    ...
    public function insertInTo($identity = true)
    {
        ...
        foreach ($this->fields as $value) {
            ...
            if (!$this->prepare) {
                ...
                $this->paramsTypes .= $this->prepareGetType($value);
                array_push($this->prepareValues, $value);
                $valor = '?';
            }
    ...
    /*     * metodo secure de eliminacion opr id
     * @param $id
     * @param $value
     */
    ...
    public function getTableAllRecords($fiels, $where = false, $order = false)
    {
        ...
        $this->concatSql($this->getTable());
        if ($where)
            $this->concatSql(" where " . $this->escapeString($where));
        if ($order)
            $this->concatSql(" ORDER BY " . $order);
    ...
    public function dataInsert($pref, $sep, $table, $vars)
    {
        ...
        while (list($key, $value) = each($vars)) {
            ...
            $value = Form::getVar($key, $vars, false); ///no aplica la seguridad
